...
void printGraph(vector<list<int>>& listGraph, int fd){
    ostringstream oss;
...
void newEdge(vector<list<int>>& listGraph, int v, int u, int fd){
    ostringstream oss;
    
    listGraph[v-1].push_back(u-1);
    oss << "Edge " << v << "," << u << " added" << endl;
    sendToClient(fd, oss.str());
}

void removeEdge(vector<list<int>>& listGraph, int v, int u, int fd){
...
vector<list<int>> newGraph(int n, int m, int fd){
    ...
    for (size_t i = 0; i < m; i++) {
       char buf[256];
        ...
        newEdge(listGraph, v, u, fd); // Update graph with new edge
    }
    return listGraph;
}
...
void kosarju(vector<list<int>>& listGraph, int fd){
    ...
    for (size_t i = 1; i <= number; i++)
    {
        ...
        {
            if(SCCs[j] == i){
                oss << j+1 << " "; // Convert to 1-based index
            }
        }
        oss << endl;   
    }
...
void handleCommand(const string& command, int fd){
    ...
    if(cmd == "Newgraph"){
        ...
        oss << "New graph created" << endl;
        sendToClient(fd, oss.str());
        printGraph(listGraph, fd);
    }
        ...
        int v, u;
            char comma;
            iss >> v >> comma >> u;
            removeEdge(listGraph, v, u, fd);
            sendToClient(fd, oss.str());
            printGraph(listGraph, fd);
    }

    else if(cmd == "Kosaraju"){
        kosarju(listGraph, fd);
    }
}
